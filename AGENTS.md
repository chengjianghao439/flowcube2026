...
- 补打中心 = **打印记录**（2026-09-14 用户决定）：入库/出库/物流三类都只列**真的生成过打印任务**的对象（入库按容器、出库按箱贴取最近一条 `print_jobs`，物流本就取自 `print_jobs`）。从未打印过的容器/箱子不出现，「未生成任务」状态与筛选项已移除；`reprintInboundBarcode` / `reprintOutboundBarcode` 同口径拒绝无记录的对象（`PRINT_BARCODE_NO_PRINT_RECORD`），避免从列表给一个从没打过标签的对象凭空造任务（生产误操作：从未打印过的塑料盒 `B000001` 被从补打中心打了出去）。从未打印过的对象要在业务单据侧补打——收货订单详情有整单/明细/条码补打；**退货任务目前没有补打入口（已知缺口，待补）**，其无打印机的提示文案已改为不承诺具体入口。另修：出库计数查询把最新任务子查询别名写成 `j` 而状态条件按 `pj` 拼，一带状态筛选就报 `Unknown column 'pj.status'`，已统一为 `pj`。回归见 `tests/print-queue.smoke.test.js`；细节见 `docs/barcode-reprint-scope-2026-09-14.md`。
