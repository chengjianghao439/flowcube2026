...
- ERP 公共页面采用紧凑 PageHeader、可换行 ActionBar、SectionCard 和轻边框 FilterCard；双列查询内容复用 `QueryFormLayout`，保持顶部对齐和内部滚动。DataTable 的自定义复合内容允许换行，默认纯文本仍可截断查看；操作列与其他列一起横向滚动，不再固定吸附；极窄操作列的按钮在格内滚动，避免越界重叠。系统所有业务列表和查找弹窗取消页面分页：不显示页码、每页条数、跳页、上一页/下一页或加载更多。GET 数据层自动按有界批次取齐标准列表后交给页面，静态 ListSummary 只显示总数；全局搜索自动读完各类游标。旧链接的 page 不限制结果；请求全程固定登录会话和会计账套，保持原筛选/仓库范围，批次失败、总数变化或重复批次明确报错，不把首批当全量。审批待办按真实 COUNT 与有界批次取齐，通知审批数量同口径，不再以旧引擎前 100 条计算总数。首页摘要与 Top 排行保留既定摘要含义；打印纸张分页不属于列表分页。实现与验证见 `docs/all-record-lists-2026-09-06.md`。BaseCrudPage 请求失败显示重试状态。Dialog 长内容限制在窗口内滚动。PC 商品列表与明细按编码、供应商型号、型号、名称、颜色独立展示，列宽不足时区域内横向滚动，禁止重新合并为商品 / 规格单元格；单商品摘要采用带标签的分格布局；具体范围见 `docs/product-columns-2026-09-05.md`，不改单位、数量或金额逻辑。覆盖范围及实际验收边界见 `docs/erp-frontend-ui-2026-09-05.md`，公共组件覆盖不代表全部页面工作流已验收。
...
- 入队 copies 必须为 1–100 的数字整数，缺省为 1；桌面将多份内容组装为一次 RAW 提交。单份保留原始模板，模板含 ^PQ 时拒绝再叠加任务多份，避免数量相乘；提交后核销失败只重试回执。
- ZPL 业务字段统一清洗 ^/~ 和控制字符，可信模板正文保留；变量仅单次回调替换，不递归展开。条码保留合法首尾空格。画布纸高按现有 203dpi 输出 ^LL，与宽度几何口径一致；300dpi 与真实走纸仍需设备验收。
