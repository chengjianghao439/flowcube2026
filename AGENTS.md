...
- **push main 后 `Deploy Browser App` 长时间 pending 是已知并发形态**（2026-09-18 v0.9.23 实操）：它与 `Build PDA APK` 共用 `flowcube-server-deploy` 组，而 PDA 的 build job 内含「等本提交浏览器部署」，PDA 先拿到组时两者互等（PDA 上限 25 分钟）。处置：`gh run cancel <pda-run-id>` 让路 → 等浏览器部署 success → 打 tag → 用 `checkout_ref` 补跑 PDA。`deploy-browser.yml` 的服务器端 `flock` 已由 300 秒对齐到 1800 秒；彻底解法（把 PDA 发布拆成独立 job）留待专门一轮实施与验证。
