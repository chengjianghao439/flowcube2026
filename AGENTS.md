...
- **只读路径不得逐行单查（N+1）**：出库前置校验曾对每条明细各查一次 `scan_logs` 聚合、`resolveReceiptParty` 曾对每张对账单/每条账款各查一次，现各改为一次 `GROUP BY` / `IN (?)`；**事务内按固定顺序逐行 `FOR UPDATE` 加锁是刻意设计，不在判定范围**（批量改写会破坏锁顺序）。批量版的取值必须与原逐条版逐一等价——例如无扫码行的明细要按 0 处理（原 `COALESCE(SUM(qty),0)` 是聚合查询，无匹配行也返回 0），否则会把「未扫码」误判成通过。`npm run test:query-loop` 机械守住「循环体内查询且所在函数无写语句/无行锁」，正当形态须在白名单逐条写明理由（白名单不被命中即失败）；**行为侧**由 `npm run test:warehouse-scan-closure` 用 stub `conn`（不连库）守住——「明细存在但一条扫码记录都没有」必须拒绝，因为 `GROUP BY` 之后这类明细在结果里根本不出现，若改写成「缺失就跳过 / 只在命中时比较」就会让出库闸门 fail-open，而 `smoke:*` 走的都是有扫码行的正常路径、不会红（`backend/src/modules/warehouse-tasks/warehouse-tasks.helpers.js`、`backend/src/modules/payments/party-identity.js`）
...
- **桌面发布必须清理服务器侧中转目录**：`/tmp/flowcube-desktop-release/${tag}` 只服务本次上传，必须在创建目录**之前**注册 `EXIT` trap 清理（成功/scp 失败/发布失败三条路径都要覆盖）。2026-09-19 实测它从不清理，v0.9.13–v0.9.25 在服务器累积 1.3G；`/tmp` 与 `/opt/flowcube` 同处根分区，把上传前的 6144MB 余量门禁吃到只剩 24MB，连续 4 轮部署在上传前被拒。`tests/deployment-resources.test.js` 机械守住（反向验证：去掉 trap、cleanup 不真删、trap 挪到 `mkdir` 之后都必须失败）（`build-desktop.yml`）
- **部署磁盘预检失败必须自解释**：上传前的 `df -Pm /tmp APP_PATH ≥ 6144MB` 预检若写成 `test "$(df …)" -ge 6144`，余量不足时 ssh 只返回 1、一行输出都没有，与 SSH 故障无法区分（2026-09-19 连续 3 轮部署因此查不出原因，见 `docs/deploy-disk-precheck-2026-09-19.md`）。`tests/deployment-resources.test.js` 机械守住「先打印余量再判定、三类失败可区分」；低磁盘时人工清理后重跑，不得自动 prune 或清回退镜像
- **生产机上批量删除/清理前先确认云盘 IO，并分批执行**：该机云盘有已知读写受限（2026-09-05 官方确认）。2026-09-19 一次删除约 1.9G（桌面发布残留 1.3G + 隔离目录 630M + `journalctl --vacuum-size=200M`）后，系统进入约 6 分钟极端缓慢——HTTPS 超时、SSH 卡在 banner exchange、cron 每分钟任务被延迟到下一分钟并跳过（`ping` 仍 17ms 正常、无 OOM/IO error/hung task 记录），只能由用户在控制台重启恢复。规范：动手前先 `iostat -x 1 2`（`%util`/`await`/`aqu-sz`）与 `vmstat`（`wa`）确认余量；分批删除（每批 ≤200MB、批间隔数秒）并用 `ionice -c3 nice -n19`；避开 MySQL 写入高峰与部署进行中；控制台级恢复请用户操作（本机无控制台凭据）。经过与判断见 `docs/deploy-disk-precheck-2026-09-19.md`
...
- **三个部署 workflow 的并发组必须同名并去掉 `cancel-in-progress`**（`flowcube-server-deploy`）：不同 group 等于互不排队，会同时 SSH 抢服务器 `/tmp/flowcube-deploy.lock`；服务器端 `flock` 只作兜底。`build-desktop` 在 push main 时只做验证构建（不碰服务器），故按**事件**分 group——tag/手动触发走 `flowcube-server-deploy`，push main 走 `build-desktop-ci`，否则会把真正的部署挤进 pending 并相互取消。发版仍走串行节奏：等浏览器部署成功再打 tag；失败用 `gh run rerun <run-id> --failed` **逐个**重跑。慢盘上 `docker load` 首次加载需 1800 秒，镜像上传（scp）时限必须与之一致，Deploy 步骤 105 分钟 / job 165 分钟（`tests/deployment-resources.test.js` 守住，见 §0.1）。事故经过见 `docs/release-v0.9.17-result.md`。
...
| 2026-09-19 部署磁盘预检静默失败（服务器低磁盘阻塞） | `docs/deploy-disk-precheck-2026-09-19.md` |
