...
用户授权所有改动提交并发布；本版整合运营优化分支、正式采用的官网第一版、候选参考源码及未合入的 v0.9.13 发布记录。用户随后明确删除未合入的 Windows 签名功能，已归档并移除其工作树改动；本版沿用既有未签名 Windows 构建策略，不加入签名配置、脚本或强制签名门禁。签名不等同于下载摘要校验，原 HTTPS、SHA-256、同 SHA CI 与发布门禁保持。工作树覆盖、验证和发布状态见 `docs/release-v0.9.15-result.md`，不能把本地候选验证当作已上线。
