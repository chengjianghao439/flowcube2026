...
- ERP 仪表盘默认首屏为待处理销售、今日出库、未清应收、待我审批四项紧凑摘要，下面为业务待办、风险与快捷发起、销售趋势和应收到期分布。待处理销售统计状态 1/2/6，执行中单独列示；默认待办先列出待实物归还、授信待审批，再按订单创建时间升序排列，日期标注为创建时间而非阶段耗时；可切换状态总览。`focus=pending` 的列表和导出只包含活动订单及仍待归还的订单。待办销售计数与列表使用相同仓库范围（含行级分仓排除），跳转携带 `range=all` 查看全部日期。应收到期分布基于真实未清余额，按北京今天分为逾期、今日、未来 7 天、7 天以后、未知到期日；与既有账龄的空到期日回退口径分开，不能伪造历史趋势。销售趋势提供订单金额、已出库订单金额和订单数三项切换，沿用报表按订单创建月份的口径，金额与单数不共轴。低库存按商品/仓库汇总实物量 ≤ 10，显示完整总数并分页查看；授信超限可分页查看超限客户并跳转销售订单，入口保持各自权限。编辑态保留自由网格，阅读态应用已保存宽度。SKU 总数、库存总量、趣味工具和版本等静态信息不占默认布局。仍同时显示三项旧趣味组件，或与上一版默认可见组件顺序和宽度完全一致的布局自动迁移；其他个性化排序和宽度保留。组件经注册表合并时安全移除下线项。
- 销售订单列表的状态快捷筛选、单号搜索和高级查询共用 URL 参数，切换筛选必须回到第一页；页面显示的订单总数必须来自服务端分页总数，不能用当前页行数冒充全量统计。状态标签计数忽略当前状态筛选，但保留其余筛选和权限范围；默认最近七天按北京时间计算，`range=all` 不自动套日期。列表按基本单位分别展示订单/已占/已派发/已出库量，不混加件和米，未占量不能直接标记为缺货。点击单号按需加载右侧预览，展示商品快照、所有仓库任务和待归还/授信标记，完整详情与占库沿用既有入口；紧凑/舒适密度在本机持久化。预览支持当前页上一单/下一单并高亮对应行，首尾禁用越界，切单重新加载详情和复位滚动。正常履约时关注列留空，避免重复状态；未生成应收记录统一显示“未生成应收”，不使用客户结算回退值制造未付或逾期。销售单新建、编辑和执行期改单共用订单信息、商品明细、订单汇总的视觉结构；首屏表单与详情摘要采用紧凑密度，常见桌面窗口应直接露出商品明细区域。占库使用专用宽弹窗，必须同时展示商品编码、供应商型号、型号、颜色、单位、需求/已占/待占数量及仓库的实物、已占、预计和可承诺量（ATP），库存不足或数量非法时禁用确认。发货弹窗按行展示完整商品身份、仓库、已占未发量和本批数量；列表页发货、取消占库只能进入详情核对，不直接执行。收货人/部门和联系电话最多 30 字符，电话支持手机、座机与国际号码，详细地址最多 200 字符。详情摘要、详情选项卡以及释放、缺货、查询、地址簿弹窗保持同一主题层级，不能为改样式复制或改写业务状态判断。
