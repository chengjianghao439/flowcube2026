...
- 生产页面门禁依赖受限验收账号：2026-09-08 的 v0.9.10 首次门禁因既有 `smoke_limited` 已删除而失败。按用户仅本次授权临时恢复、重跑通过后已重新删除并撤销会话；后续发布须先核对账号条件，不能默认永久启用、自动恢复已删除账号或跳过权限验证。应后续完善受限账号配置和临时生命周期；本次发布/恢复证据见 `docs/release-v0.9.10-result.md`。
