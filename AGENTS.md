...
- PC 壳层与辅助界面：登录页使用真实业务说明和统一表单，不展示模拟业务卡片或无动作链接；登录行为与目标回跳沿用认证逻辑。全局搜索结果按类型分组并允许长文本换行；通知中心复用 Popover，使用统一语义色和可滚动条目，保留原通知优先级。商品/库存导入共用 ImportSteps 排版，原有上传时机及处理器保持不变。采购申请、处置和报销驳回原因采用多行输入，保留原限长及提交规则；抽屉和确认层沿用主题语义色。全前端收口报告见 `docs/pc-ui-final-audit-2026-09-05.md`，静态覆盖索引见 `docs/pc-ui-source-inventory.md`，可用 `node scripts/pc-ui-inventory.cjs` 重新扫描；索引数量不等于全部状态已验收。
...
- ERP 公共页面采用紧凑 PageHeader、可换行 ActionBar、SectionCard 和轻边框 FilterCard；双列查询内容复用 `QueryFormLayout`，保持顶部对齐和内部滚动。DataTable 的自定义复合内容允许换行，默认纯文本仍可截断查看；固定操作列保持不透明底色。通用 Pagination 有记录时即展示服务端总数，单页前后翻页禁用；分页方向复用 PaginationArrow 的 Lucide 左右箭头，保留中文悬停和无障碍标签，商品查找及会计独立分页同步使用；BaseCrudPage 请求失败显示重试状态。Dialog 长内容限制在窗口内滚动。PC 商品列表与明细按编码、供应商型号、型号、名称、颜色独立展示，列宽不足时区域内横向滚动，禁止重新合并为商品 / 规格单元格；单商品摘要采用带标签的分格布局；具体范围见 `docs/product-columns-2026-09-05.md`，不改单位、数量或金额逻辑。覆盖范围及实际验收边界见 `docs/erp-frontend-ui-2026-09-05.md`，公共组件覆盖不代表全部页面工作流已验收。

- 销售分组桌面样式覆盖销售管理、销售退货、超额放行申请、物流运单、客户管理、承运商管理和客户对账门户。列表身份信息复用 `RecordIdentity`，表单按资料与业务配置分区；超额申请驳回使用统一原因弹窗、300 字限制及提交中禁用，沿用原审批接口；销售退货保持新建和详情能力，不虚构编辑入口。客户对账门户未选客户只显示一个引导空态，选择后展示只读列表及错误重试。物流当前显示数量明确最多 500 张，不当作全量总数。覆盖记录见 `docs/sales-group-ui-2026-09-05.md`。

- 全局商品查找统一复用 `ProductFinderModal`，保留单选回填。默认查询只展示商品身份、规格、单位和分类；销售上下文增加基础参考售价，采购上下文显示供应商；仅传入仓库时显示该仓可用库存，命名仓库并说明不含预计到货，不能作为 ATP 或出库承诺。搜索覆盖名称、编码、条码、供应商型号、型号、颜色；分类名称可选父分类及全部子分类，展开按钮独立。按 30 条服务端分页并显示真实总数，商品名称与 ID 共同稳定排序；查询、分类、页码、仓库或场景变化清空选择，加载中或失败禁用确认，关闭后重置草稿。选中区显示完整分类路径、供应商、条码；价格和库存以订单业务校验为准。接口新增可空 barcode，保持既有价格、库存投影和权限口径。实现与验证见 `docs/product-finder-redesign-2026-09-05.md`。

- ERP 仪表盘默认首屏为待处理销售、今日出库、未清应收、待我审批四项紧凑摘要，下面为业务待办、风险与快捷发起、销售趋势和应收到期分布。待处理销售统计状态 1/2/6，执行中单独列示；默认待办先列出待实物归还、授信待审批，再按订单创建时间升序排列，日期标注为创建时间而非阶段耗时；可切换状态总览。`focus=pending` 的列表和导出只包含活动订单及仍待归还的订单。待办销售计数与列表使用相同仓库范围（含行级分仓排除），跳转携带 `range=all` 查看全部日期。应收到期分布基于真实未清余额，按北京今天分为逾期、今日、未来 7 天、7 天以后、未知到期日；与既有账龄的空到期日回退口径分开，不能伪造历史趋势。销售趋势提供订单金额、已出库订单金额和订单数三项切换，沿用报表按订单创建月份的口径，金额与单数不共轴。低库存按商品/仓库汇总实物量 ≤ 10，显示完整总数并分页查看；授信超限可分页查看超限客户并跳转销售订单，入口保持各自权限。编辑态保留自由网格，阅读态应用已保存宽度，按今日重点、采购与销售、库存与仓储、财务与资金、个人工具和系统信息分区，区内保留个人排序；指标卡、业务卡、个人小工具分别使用合适高度。提供“展示全部卡片”入口，进入布局草稿后保存到当前账号，仅挂载有权限的组件；不改变其他账号或新用户的精简默认。SKU 总数、库存总量、趣味工具和版本等静态信息不占默认布局。仅与上一版默认可见组件顺序和宽度完全一致的布局自动迁移；不再根据三张趣味卡片判定旧默认，避免全部展示或个人组合被重置。其他个性化排序和宽度保留。组件经注册表合并时安全移除下线项。
- 销售订单列表按 9 月 4 日前的普通表格布局恢复：单号、客户、仓库、折后金额、备注、状态、回款状态、经办人、创建时间、操作各自独立列；页头为说明及导出、查询、新建操作，标题下仅增加无数量的状态快捷分类。列表不显示运输列、单号搜索框、行距工具或快速预览侧栏，承运商仍在订单详情查看。恢复布局只调整呈现，保留现行折后金额、权限、状态、分页及发货核对规则，不能直接覆盖历史文件回退业务修复。状态分类与查询弹窗共用 URL 参数，切换状态保留其他条件并回到第一页；分页总数来自服务端，默认最近七天按北京时间计算，`range=all` 不套日期。采购、销售、收货共用标题下方的 OrderStatusFilter。未生成应收记录仍显示“未生成应收”，不使用客户结算回退值制造未付或逾期。新建、编辑、改单汇总及占库摘要按基本单位分别折算，不混加件和米；未占量不直接标记为缺货。恢复依据与验证见 `docs/sale-classic-layout-2026-09-05.md`。销售单新建、编辑和执行期改单共用订单信息、商品明细、订单汇总的视觉结构；商品区域复用 SaleOrderItemsSection，商品编码、供应商型号、型号、名称和颜色各自独立成列，数量与单价使用独立录入列；汇总使用紧凑横向条，高级查询按订单条件、日期与备注分组。首屏表单与详情摘要采用紧凑密度，常见桌面窗口应直接露出商品明细区域。占库使用专用宽弹窗，必须同时展示商品编码、供应商型号、型号、颜色、单位、需求/已占/待占数量及仓库的实物、已占、预计和可承诺量（ATP），库存不足或数量非法时禁用确认。发货弹窗按行展示完整商品身份、仓库、已占未发量和本批数量；列表页发货、取消占库只能进入详情核对，不直接执行。收货人/部门和联系电话最多 30 字符，电话支持手机、座机与国际号码，详细地址最多 200 字符。详情摘要、详情选项卡以及释放、缺货、查询、地址簿弹窗保持同一主题层级，不能为改样式复制或改写业务状态判断。取消占库明细同时显示商品与仓库，数量错误给出明确说明；“选中全部已占数量”只填充选择，仍需确认释放。销售详情统一商品身份列名称，金额汇总横向对照商品金额、折扣和净额；装箱汇总按明细行数展示，不混加不同单位数量；任务数不能解释为不同仓库数，操作记录显示事件说明。超额放行列表展示本单净额、授信额度、超额金额和申请/驳回原因，金额明确为申请时快照，不解释为当前额度。
