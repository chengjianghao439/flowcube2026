...
- 补打中心「入库条码」范围（2026-09-14）：它是**条码打印状态全量查询 + 丢失补打**，不是失败清单——默认无状态筛选时列出全部库存容器，其中没有打印任务的显示「未生成打印任务」且仍可补打（收货时无可用打印机的容器就靠这个入口补打，必须保留）。**塑料盒（`container_type=2` / `B` 码）例外**：ERP 新建空盒不入队标签，因此只列出真的进过打印队列的塑料盒（PDA 拆分勾选「打印新塑料盒条码」那种），无任务的塑料盒既不列出、直接调 `reprintBarcodeRecord` 也会被 `PRINT_BARCODE_PLASTIC_BOX_NOT_PRINTED` 拒绝，避免从列表凭空造任务。回归见 `tests/print-queue.smoke.test.js`；起因与验证见 `docs/barcode-reprint-scope-2026-09-14.md`。
