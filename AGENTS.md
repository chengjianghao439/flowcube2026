...
**回归守卫**：`npm run smoke:audit-2026-09-18`（29 条，DB）、`npm run smoke:label-render-degrade`（3 条，DB）、`npm run test:sale-revenue-discount`（7 条，纯函数）、`npm run test:route-permission-contract`（纯静态）、`npm run test:inventory-lock-order`（纯静态）与 `npm run test:sql-placeholder`（纯静态），**均已接入 `.github/workflows/test.yml`**（不再新增孤儿测试）。★ 用例已逐条验证「回退修复后必然失败」：`Missing expected rejection`、`被锁定容器必须保持 ACTIVE`、`1122 应等于 毛额 − 整单折扣`、`RESERVE_WAREHOUSE_CHANGE_NOT_ALLOWED` 缺失、`WAREHOUSE_SCOPE_DENIED` 缺失、`{"pl":"current","year":"stale"}`（P0-2 / P1 三批 / P2 两批各自实测过回退失败）；反向对照用例（正常撤销 / 真盘亏照扣 / 同仓补占 / 同仓移库 / 非超管改他人范围 / 范围正确时不误拦 / 豁免腐烂检测）修复前后都通过，确认测试没有过度约束。
