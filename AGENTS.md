...
- 打印记录页（补打中心）口径 = **只记唯一码**（2026-09-14 用户规则）：条码指向一个唯一对象才留记录、丢了能按记录补打。入库/出库分别要求容器/箱贴最近一条 `print_jobs` 存在，物流本来就取自 `print_jobs`；从未打印过的不出现，「未生成任务」状态与筛选项已移除，`reprintInboundBarcode` / `reprintOutboundBarcode` 同口径拒绝无记录对象（`PRINT_BARCODE_NO_PRINT_RECORD`）。**排除可复用固定码**：塑料盒自身的码（`source_ref_type='plastic_box_create'`，即使打过标签也不进，直接调接口返回 `PRINT_BARCODE_NOT_UNIQUE`）请在「塑料盒」页用新增的「打印条码」（`POST /api/plastic-boxes/:id/print-label`）重复打印；货架/库位标签本就不在本页取数，各页有自己的打印入口。**注意两个 `B` 码不同**：拆分散货（`container_split` / `sale_order_adjustment_return`）每次新建、指向唯一一批货，**照常记录**；只有空盒是复用码。列表只要求「有打印记录」，不要求属于收货单，退货等来源的容器只要打过标签同样能重打。订单类页面的打印进度：后端已为 `inbound`/`sale-return`/`sale`/`wave` 产出 `print` 分组（`purchase-return` 无），前端已全部露出「条码打印」标签（销售订单本次补上），供客服跟踪进度。另修：出库计数查询把最新任务子查询别名写成 `j` 而状态条件按 `pj` 拼，一带状态筛选就报 `Unknown column 'pj.status'`，已统一为 `pj`。回归见 `tests/print-queue.smoke.test.js`；细节与剩余缺口见 `docs/barcode-reprint-scope-2026-09-14.md`。
