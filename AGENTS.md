...
- **发版收尾的两个实测坑（2026-09-18 v0.9.20）**：① `Build PDA APK` 的 push 触发带路径过滤（`frontend/**`、`backend/apk/version.json` 等），**只动测试/文档的后续提交不会触发 PDA 构建**，且首次提交的 PDA 构建若因该 SHA 浏览器部署失败而失败，最终发布 SHA 上就没有 PDA 产物（v0.9.19 就是这样把生产 PDA 停在 0.9.18/126 的）——此时在最终 SHA 上 `gh workflow run build-pda-apk.yml --ref main` 补跑，它仍会等待本 SHA 的浏览器部署成功再发布。② `Build Desktop Installer` 的「Upload EXE to Release」在 Windows runner 上可能长时间挂起（v0.9.20 连续两次卡住 15 分钟以上，而 v0.9.18 同步骤上传同一个 112 MB exe 只要 10 秒）：`latest.json` 与服务器正式包由该步骤**之前**的步骤写入，用户更新链路不受影响；处置是取消挂起 run（释放 `flowcube-server-deploy`，避免阻塞排队的 PDA），把服务器 `/versions/v<版本>/` 的 **CI 构建包**取回复算 sha256 与 `latest.json` 一致后用 `gh release upload <tag> <file> --clobber` 补传——**禁止在本机重新构建或复制本机产物**；被中断的 `gh release create` 会留下**草稿** Release，需 `gh release edit <tag> --draft=false --title "FlowCube ERP <版本>" --latest` 才正式对外。结果与核验记录见 `docs/release-v0.9.20-result.md`。
