...
export const routeRegistry: RouteRegistryEntry[] = [
  ...
  },
  {
    ...
    keepAlive: true,
    tabIdentity: pathnameIdentity,
    nav: { kind: 'menu', group: '销售', section: '对外查询', order: 50 },
  },
  {
    path: '/carriers',
    title: '承运商管理',
...
export const routePatterns: RoutePatternEntry[] = [
  ...
  {
    // 2026-09-17 验收修复（G-10）：新建页必须是"创建权限"，不能沿用查看权限——
    // 否则只读账号直接输 URL 就能打开完整新建表单（实测 /sale/new 等 7 个模块均如此）。
  ...
  {
    pattern: /^\/products\/\d+$/,
    title: (path) => `编辑商品 #${path.split('/').pop()}`,
    permission: PERMISSIONS.PRODUCT_VIEW,
    componentKey: 'ProductFormPage',
    keepAlive: true,
    tabIdentity: pathnameIdentity,
  ...
  {
    pattern: /^\/purchase\/\d+$/,
    title: (path) => `采购订单 #${path.split('/').pop()}`,
    permission: PERMISSIONS.PURCHASE_ORDER_VIEW,
    componentKey: 'PurchaseFormPage',
    keepAlive: true,
    tabIdentity: pathnameIdentity,
  ...
  {
    pattern: /^\/purchase-requisitions\/new$/,
    title: () => '新建采购申请单',
    permission: PERMISSIONS.PURCHASE_REQUISITION_CREATE,
  ...
  {
    pattern: /^\/purchase-requisitions\/\d+$/,
    title: (path) => `采购申请单 #${path.split('/').pop()}`,
    permission: PERMISSIONS.PURCHASE_REQUISITION_VIEW,
    componentKey: 'RequisitionFormPage',
    keepAlive: true,
    tabIdentity: pathnameIdentity,
  ...
  },
  {
    ...
    title: () => '新建调拨单',
    permission: PERMISSIONS.TRANSFER_ORDER_CREATE,
  ...
  {
    pattern: /^\/returns\/purchase\/\d+$/,
    title: (path) => `采购退货单 #${path.split('/').pop()}`,
    permission: PERMISSIONS.RETURN_ORDER_VIEW,
    componentKey: 'PurchaseReturnFormPage',
    keepAlive: true,
    tabIdentity: pathnameIdentity,
  ...
  {
    pattern: /^\/returns\/sale\/\d+$/,
    title: (path) => `销售退货单 #${path.split('/').pop()}`,
    permission: PERMISSIONS.RETURN_ORDER_VIEW,
    componentKey: 'SaleReturnFormPage',
    keepAlive: true,
    tabIdentity: pathnameIdentity,
