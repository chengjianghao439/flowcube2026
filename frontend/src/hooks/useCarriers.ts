...
export const useCarriers       = (params: object) => useQuery({ queryKey: ['carriers', params], queryFn: () => getCarriersApi(params).then(r => r.data.data!) })
export const useCarriersActive = ()               => useQuery({ queryKey: ['carriers-active'],  queryFn: () => getCarriersActiveApi().then(r => r.data.data || []) })
