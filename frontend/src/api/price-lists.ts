...
export const getCustomerPriceApi     = (customerId:number, productId:number) => client.get<ApiResponse<CustomerResolvedPrice|null>>('/price-lists/customer-price', { params:{customerId,productId} })
export const bindCustomerApi         = (customerId:number, priceLevel:'A'|'B'|'C'|'D') => client.put<ApiResponse<null>>('/price-lists/bind-customer', { customerId, priceLevel })
