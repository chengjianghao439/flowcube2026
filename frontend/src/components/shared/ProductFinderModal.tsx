    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex min-h-0 flex-1">
        <aside aria-label="商品分类" className="w-52 shrink-0 overflow-y-auto border-r px-3 py-3">
          ...
          {categories.isError ? <div className="px-2 py-4 text-xs text-muted-foreground">分类加载失败<Button size="sm" variant="ghost" onClick={() => void categories.refetch()}>重试分类</Button></div> : categories.isLoading ? <p className="px-2 py-4 text-xs text-muted-foreground">正在加载分类…</p> : <CategoryTree nodes={categories.data ?? []} selectedId={category?.id ?? null} onSelect={chooseCategory} />}
