...
function InboundCard({ task, onTap }: { task:InboundTask; onTap:()=>void }) {
  ...
  return (
    <SectionCard title={task.taskNo} actions={<Badge variant={STATUS_VARIANT[task.status]}>{INBOUND_STATUS_LABEL[task.status]}</Badge>}>
      <div className="space-y-3">
        <div>
          <p className="font-semibold text-foreground">{task.supplierName ?? '未知供应商'}</p>
