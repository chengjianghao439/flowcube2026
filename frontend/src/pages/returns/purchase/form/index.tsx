...
function FormView({ closeTab, tabPath }: { closeTab: () => void; tabPath: string }) {
  ...
  function handleFinderConfirm(product: ProductFinderResult) {
    ...
    setItems(prev => prev.map(i => i._key === k
      ? { ...i, productId: product.id, productCode: product.code, productName: product.name, articleNumber: product.articleNumber ?? null, spec: product.spec ?? null, color: product.color ?? null, unit: product.unit, unitPrice: product.costPrice ?? 0 }
      : i,
    ))
