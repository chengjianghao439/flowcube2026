...
const reserve = async(req,res,next)=>{ try{const op=getOperatorFromRequest(req);const items=Array.isArray(req.body?.items)?req.body.items:[];await svc.reserveStock(+req.params.id, op, items, { confirmCreditOverride: req.body?.confirmCreditOverride === true });return successResponse(res,null,'预占库存成功')}catch(e){next(e)} }
