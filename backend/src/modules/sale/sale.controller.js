...
const update = async(req,res,next)=>{ try{const op=await getOperator(req.user.userId);await svc.update(+req.params.id,{...req.body,operator:op});return successResponse(res,null,'保存成功')}catch(e){next(e)} }
const reserve = async(req,res,next)=>{ try{const op=await getOperator(req.user.userId);await svc.reserveStock(+req.params.id, op);return successResponse(res,null,'库存占用成功')}catch(e){next(e)} }
const release = async(req,res,next)=>{ try{const op=await getOperator(req.user.userId);await svc.releaseStock(+req.params.id, op);return successResponse(res,null,'已取消占库，订单恢复为草稿')}catch(e){next(e)} }
const ship    = async(req,res,next)=>{ try{const op=await getOperator(req.user.userId);await svc.ship(+req.params.id, op);return successResponse(res,null,'出库任务已创建，等待仓库执行')}catch(e){next(e)} }
const cancel  = async(req,res,next)=>{ try{const op=await getOperator(req.user.userId);await svc.cancel(+req.params.id, op);return successResponse(res,null,'已取消')}catch(e){next(e)} }
