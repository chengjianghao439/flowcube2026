...
const list   = async(req,res,next)=>{ try{return successResponse(res,await svc.findAll({page:+req.query.page||1,pageSize:+req.query.pageSize||20,keyword:req.query.keyword||'',status:req.query.status?+req.query.status:null,productId:req.query.productId?+req.query.productId:null,customerId:req.query.customerId?+req.query.customerId:null,warehouseId:req.query.warehouseId?+req.query.warehouseId:null,startDate:req.query.startDate||null,endDate:req.query.endDate||null,remark:req.query.remark||null,operatorId:req.query.operatorId?+req.query.operatorId:null}),'查询成功')}catch(e){next(e)} }
...
const reserve = async(req,res,next)=>{ try{const op=getOperatorFromRequest(req);await svc.reserveStock(+req.params.id, op);return successResponse(res,null,'预占库存成功')}catch(e){next(e)} }
...
const ship    = async(req,res,next)=>{ try{const op=getOperatorFromRequest(req);await svc.ship(+req.params.id, op);return successResponse(res,null,'出库任务已创建，等待仓库操作')}catch(e){next(e)} }
...
module.exports = { list, detail, create, update, adjust, reserve, release, ship, cancel, del }
