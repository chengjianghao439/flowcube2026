...
router.get('/purchase/source-order', requirePermission(PERMISSIONS.RETURN_ORDER_CREATE), vQuery(sourceOrderQuery), async(req,res,next)=>{try{return successResponse(res,await svc.loadPurchaseSourceOrderByNo(req.query.orderNo),'查询成功')}catch(e){next(e)}})
...
router.post('/purchase',             requirePermission(PERMISSIONS.RETURN_ORDER_CREATE), vBody(prSchema),async(req,res,next)=>{try{const op=getOperatorFromRequest(req);return successResponse(res,await svc.createPR({...req.body,operator:op}),'创建成功',201)}catch(e){next(e)}})
router.post('/purchase/:id/confirm', requirePermission(PERMISSIONS.RETURN_ORDER_CONFIRM), vParams(idParam),async(req,res,next)=>{try{await svc.confirmPR(req.params.id, getOperatorFromRequest(req));return successResponse(res,null,'已确认')}catch(e){next(e)}})
router.post('/purchase/:id/execute', requirePermission(PERMISSIONS.RETURN_ORDER_EXECUTE), vParams(idParam),async(req,res,next)=>{try{await svc.executePR(req.params.id,getOperatorFromRequest(req));return successResponse(res,null,'退货执行成功，库存已扣减')}catch(e){next(e)}})
router.post('/purchase/:id/cancel',  requirePermission(PERMISSIONS.RETURN_ORDER_CANCEL), vParams(idParam),async(req,res,next)=>{try{await svc.cancelPR(req.params.id, getOperatorFromRequest(req));return successResponse(res,null,'已取消')}catch(e){next(e)}})
...
router.get('/sale/source-order', requirePermission(PERMISSIONS.RETURN_ORDER_CREATE), vQuery(sourceOrderQuery), async(req,res,next)=>{try{return successResponse(res,await svc.loadSaleSourceOrderByNo(req.query.orderNo),'查询成功')}catch(e){next(e)}})
...
router.post('/sale',             requirePermission(PERMISSIONS.RETURN_ORDER_CREATE), vBody(srSchema),async(req,res,next)=>{try{const op=getOperatorFromRequest(req);return successResponse(res,await svc.createSR({...req.body,operator:op}),'创建成功',201)}catch(e){next(e)}})
router.post('/sale/:id/confirm', requirePermission(PERMISSIONS.RETURN_ORDER_CONFIRM), vParams(idParam),async(req,res,next)=>{try{await svc.confirmSR(req.params.id, getOperatorFromRequest(req));return successResponse(res,null,'已确认')}catch(e){next(e)}})
router.post('/sale/:id/execute', requirePermission(PERMISSIONS.RETURN_ORDER_EXECUTE), vParams(idParam),async(req,res,next)=>{try{await svc.executeSR(req.params.id,getOperatorFromRequest(req));return successResponse(res,null,'退货入库成功，库存已增加')}catch(e){next(e)}})
router.post('/sale/:id/cancel',  requirePermission(PERMISSIONS.RETURN_ORDER_CANCEL), vParams(idParam),async(req,res,next)=>{try{await svc.cancelSR(req.params.id, getOperatorFromRequest(req));return successResponse(res,null,'已取消')}catch(e){next(e)}})
