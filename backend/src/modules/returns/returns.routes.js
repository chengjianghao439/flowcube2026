...
router.post('/purchase/:id/confirm', requirePermission(PERMISSIONS.RETURN_ORDER_CONFIRM), vParams(idParam),async(req,res,next)=>{try{await svc.confirmPR(req.params.id, await getOp(req.user.userId));return successResponse(res,null,'已确认')}catch(e){next(e)}})
...
router.post('/purchase/:id/cancel',  requirePermission(PERMISSIONS.RETURN_ORDER_CANCEL), vParams(idParam),async(req,res,next)=>{try{await svc.cancelPR(req.params.id, await getOp(req.user.userId));return successResponse(res,null,'已取消')}catch(e){next(e)}})
...
router.post('/sale/:id/confirm', requirePermission(PERMISSIONS.RETURN_ORDER_CONFIRM), vParams(idParam),async(req,res,next)=>{try{await svc.confirmSR(req.params.id, await getOp(req.user.userId));return successResponse(res,null,'已确认')}catch(e){next(e)}})
...
router.post('/sale/:id/cancel',  requirePermission(PERMISSIONS.RETURN_ORDER_CANCEL), vParams(idParam),async(req,res,next)=>{try{await svc.cancelSR(req.params.id, await getOp(req.user.userId));return successResponse(res,null,'已取消')}catch(e){next(e)}})
