...
router.post('/purchase/:id/confirm', requirePermission(PERMISSIONS.RETURN_ORDER_CONFIRM), vParams(idParam),async(req,res,next)=>{try{await svc.confirmPR(req.params.id);return successResponse(res,null,'已确认')}catch(e){next(e)}})
...
router.post('/sale/:id/confirm', requirePermission(PERMISSIONS.RETURN_ORDER_CONFIRM), vParams(idParam),async(req,res,next)=>{try{await svc.confirmSR(req.params.id);return successResponse(res,null,'已确认')}catch(e){next(e)}})
