...
const create  = async(req,res,next)=>{ try{const op=await getOp(req.user.userId);return successResponse(res,await svc.create({...req.body,operator:op}),'创建成功',201)}catch(e){next(e)} }
...
const submit  = async(req,res,next)=>{ try{await svc.submit(+req.params.id,await getOp(req.user.userId));return successResponse(res,null,'盘点已提交，库存已同步调整')}catch(e){next(e)} }
