...
const submit  = async(req,res,next)=>{ try{await svc.submit(+req.params.id,getOperatorFromRequest(req),req.user?.warehouseIds??null);return successResponse(res,null,'盘点已提交，库存已同步调整')}catch(e){next(e)} }
