...
const saveItemScans = async(req,res,next)=>{ try{const data=await svc.saveItemContainerScans(+req.params.id,+req.params.itemId,req.body.scans,getOperatorFromRequest(req),req.user?.warehouseIds??null,require('../../utils/requestKey').extractRequestKey(req));return successResponse(res,data,`已记录 ${data.scannedContainers} 个条码`)}catch(e){next(e)} }
