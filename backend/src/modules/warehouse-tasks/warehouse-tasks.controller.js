...
const ship = async(req,res,next)=>{ try{const taskId=+req.params.id;const data=await svc.ship(taskId,getOperatorFromRequest(req),await svc.getShipContext(taskId),{requestKey:extractRequestKey(req),serialNosByProduct:(req.body&&req.body.serialNosByProduct)||null});return successResponse(res,data,'出库成功')}catch(e){next(e)} }
...
const confirmAdjustmentContainerReturn = async(req,res,next)=>{ try{const targetLocationId=req.body?.targetLocationId?+req.body.targetLocationId:null;const serialNos=Array.isArray(req.body?.serialNos)?req.body.serialNos:null;const result=await svc.confirmContainerReturn(+req.params.returnId,{targetLocationId,serialNos,operator:getOperatorFromRequest(req)});return successResponse(res,result,result.finalized?'改单已全部确认完成':'已确认归还')}catch(e){next(e)} }
