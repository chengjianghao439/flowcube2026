...
const listAllWarehouses = async(req,res,next)=>{ try{const{keyword='',status,warehouseId}=req.query;const data=await svc.findAllWarehouses({keyword,status:status?+status:null,warehouseId:warehouseId?+warehouseId:null});return successResponse(res,data,'查询成功')}catch(e){next(e)} }
