...
const list = async(req,res,next)=>{ try{return successResponse(res,await svc.findAll({page:+req.query.page||1,pageSize:+req.query.pageSize||20,keyword:req.query.keyword||'',status:req.query.status?+req.query.status:null,productId:req.query.productId?+req.query.productId:null,warehouseId:req.query.warehouseId?+req.query.warehouseId:null,operatorId:req.query.operatorId?+req.query.operatorId:null,startDate:req.query.startDate||null,endDate:req.query.endDate||null,remark:req.query.remark||null}),'查询成功')}catch(e){next(e)} }
...
const confirm = async(req,res,next)=>{ try{await svc.confirm(req.params.id,getOperatorFromRequest(req));return successResponse(res,null,'已确认并派发到 PDA')}catch(e){next(e)} }
