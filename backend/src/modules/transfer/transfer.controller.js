...
const confirm = async(req,res,next)=>{ try{await svc.confirm(req.params.id,getOperatorFromRequest(req));return successResponse(res,null,'确认成功')}catch(e){next(e)} }
const execute = async(req,res,next)=>{ try{await svc.execute(req.params.id,getOperatorFromRequest(req));return successResponse(res,null,'调拨执行成功，库存已同步')}catch(e){next(e)} }
...
module.exports = { list, detail, create, confirm, execute, cancel }
