...
router.post('/',             requirePermission(PERMISSIONS.TRANSFER_ORDER_CREATE), vBody(createSchema),async(req,res,next)=>{try{const op=await getOp(req.user.userId);return successResponse(res,await svc.create({...req.body,operator:op}),'创建成功',201)}catch(e){next(e)}})
router.post('/:id/confirm',  requirePermission(PERMISSIONS.TRANSFER_ORDER_CONFIRM), vParams(idParam),async(req,res,next)=>{try{await svc.confirm(req.params.id, await getOp(req.user.userId));return successResponse(res,null,'确认成功')}catch(e){next(e)}})
router.post('/:id/execute',  requirePermission(PERMISSIONS.TRANSFER_ORDER_EXECUTE), vParams(idParam),async(req,res,next)=>{try{await svc.execute(req.params.id,await getOp(req.user.userId));return successResponse(res,null,'调拨执行成功，库存已同步')}catch(e){next(e)}})
router.post('/:id/cancel',   requirePermission(PERMISSIONS.TRANSFER_ORDER_CANCEL), vParams(idParam),async(req,res,next)=>{try{await svc.cancel(req.params.id, await getOp(req.user.userId));return successResponse(res,null,'已取消')}catch(e){next(e)}})
