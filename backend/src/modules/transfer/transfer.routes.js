...
router.post('/:id/confirm',  requirePermission(PERMISSIONS.TRANSFER_ORDER_CONFIRM), vParams(idParam),async(req,res,next)=>{try{await svc.confirm(req.params.id);return successResponse(res,null,'确认成功')}catch(e){next(e)}})
...
router.post('/:id/cancel',   requirePermission(PERMISSIONS.TRANSFER_ORDER_CANCEL), vParams(idParam),async(req,res,next)=>{try{await svc.cancel(req.params.id);return successResponse(res,null,'已取消')}catch(e){next(e)}})
