...
async function findAll({ page = 1, pageSize = 20, keyword = '', status = null, productId = null, warehouseId = null, operatorId = null, startDate = null, endDate = null, remark = null, supplierId = null, scopeWarehouseIds = null }) {
  ...
  const [rows] = await pool.query(
    `SELECT t.* FROM inbound_tasks t WHERE ${where} ORDER BY t.created_at DESC, t.id DESC LIMIT ? OFFSET ?`,
    [...params, ps, offset],
  )
...
async function findById(id, scopeWarehouseIds = null) {
  ...
  const [items] = await pool.query(
    `SELECT iti.*, poi.unit_price, poi.remark AS purchase_item_remark,
     ...
     LEFT JOIN product_units pu ON pu.id = (
       ...
       ORDER BY pu2.sort_order, pu2.id LIMIT 1)
     WHERE iti.task_id = ?`,
    [id],
  )
