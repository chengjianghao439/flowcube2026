...
async function buildNotifications(scopeWarehouseIds = null, userId = null) {
  ...
  // 2026-09-14：补打只在「打印记录」页，收货详情不再展示打印记录，通知改为直接落到
  // 打印记录页并按该收货单过滤（仍带 taskId 便于客服对单）。
  if (inboundPrintFailures > 0) pushNotification(items, seen, { code: 'INBOUND_PRINT_FAILED', category: 'operations', priority: 20, type: 'warning', icon: '🏷️', text: `${inboundPrintFailures} 条收货条码打印失败待补打`, path: failedInboundTarget?.taskId ? `/settings/barcode-print-query?category=inbound&status=failed&inboundTaskId=${failedInboundTarget.taskId}` : '/settings/barcode-print-query?category=inbound&status=failed' })
