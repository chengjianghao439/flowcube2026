 ...
 *
 * 注意：搜索是「跨全部业务」的只读查询，不经数据权限过滤（与基础资料页一致）；
 * 若需要按用户仓库范围过滤，可在此接 scopeFilter（但会因单据类型无统一仓库列而复杂化，
 * 暂保持全量，与供应商/客户档案搜索同语义）。
 */
...
const ENTITIES = [
  ...
  { type: 'purchase',     label: '采购单',     pathBase: '/purchase',             table: 'purchase_orders',           noField: 'order_no',  subtitleField: 'supplier_name', searchFields: ['order_no'] },
  { type: 'sale',         label: '销售单',     pathBase: '/sale',                 table: 'sale_orders',               noField: 'order_no',  subtitleField: 'customer_name', searchFields: ['order_no'] },
  { type: 'requisition',  label: '采购请购',   pathBase: '/purchase-requisitions', table: 'purchase_requisitions',    noField: 'requisition_no', subtitleField: 'applicant_name', searchFields: ['requisition_no', 'title'] },
  { type: 'transfer',     label: '调拨单',     pathBase: '/transfer',             table: 'transfer_orders',           noField: 'order_no',  subtitleField: 'from_warehouse_name', searchFields: ['order_no'] },
  { type: 'purchaseReturn', label: '采购退货', pathBase: '/returns/purchase',      table: 'purchase_returns',          noField: 'return_no', subtitleField: 'supplier_name', searchFields: ['return_no'] },
  { type: 'saleReturn',   label: '销售退货',   pathBase: '/returns/sale',          table: 'sale_returns',              noField: 'return_no', subtitleField: 'customer_name', searchFields: ['return_no'] },
  { type: 'inbound',      label: '收货订单',   pathBase: '/inbound-tasks',        table: 'inbound_tasks',             noField: 'task_no',   subtitleField: 'supplier_name', searchFields: ['task_no'] },
  ...
  { type: 'disposal',     label: '呆滞处置',   pathBase: '/disposals',             table: 'inventory_disposal_orders', noField: 'disposal_no', subtitleField: 'warehouse_name', searchFields: ['disposal_no'] },
  ...
  { type: 'stockcheck',   label: '盘点单',     pathBase: '/stockcheck',            table: 'inventory_checks',          noField: 'check_no',  subtitleField: 'warehouse_name', searchFields: ['check_no'] },
...
async function searchGlobal(rawQuery) {
  ...
  await Promise.all(ENTITIES.map(async (ent) => {
    ...
    const sql = `SELECT id, ${ent.noField} AS no_val, ${ent.subtitleField} AS subtitle
                 FROM ${ent.table}
                 WHERE deleted_at IS NULL AND (${searchCond})
                 ORDER BY id DESC LIMIT 5`
    const [rows] = await pool.query(sql, ent.searchFields.map(() => like))
    for (const r of rows) {
      results.push({
        id: Number(r.id),
        type: ent.type,
