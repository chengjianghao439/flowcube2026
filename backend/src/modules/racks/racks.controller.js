...
async function detail(req, res, next) {
  try {
    const data = await racksService.findById(parseInt(req.params.id), scopeOf(req))
    return successResponse(res, data, '查询成功')
  } catch (err) { next(err) }
}
...
async function update(req, res, next) {
  try {
    const result = await racksService.update(parseInt(req.params.id), req.body, scopeOf(req))
...
async function remove(req, res, next) {
  try {
    await racksService.softDelete(parseInt(req.params.id), scopeOf(req))
