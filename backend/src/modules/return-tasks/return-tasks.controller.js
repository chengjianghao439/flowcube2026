...
const receive = async(req,res,next)=>{
  ...
  try{await conn.beginTransaction();const{productId,packages}=req.body;const result=await svc.receive(conn,+req.params.id,{productId,packages,requestKey:req.headers['x-request-key'],userId:req.user?.userId??null,pdaWarehouseId:req.pda?.warehouseId??null});await conn.commit();return successResponse(res,result)}catch(e){await conn.rollback();next(e)}finally{conn.release()}
}
