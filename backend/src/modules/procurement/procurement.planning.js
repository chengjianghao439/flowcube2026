...
function roundPurchase(net, multiple = 0, minimum = 0) {
  ...
  const required = Math.ceil(Number(net) * 100 - 1e-7)
  if (!required) return 0
  const pack = Math.round(Number(multiple) * 100)
  const floor = Math.max(required, Math.ceil(Number(minimum) * 100 - 1e-7))
  return (pack > 0 ? Math.ceil(floor / pack) * pack : floor) / 100
}
...
function calculateSupply(input = {}) {
  ...
  const netRequirement = roundQty(Math.max(0, physicalNet - provisionalCoverage))
  const suggestedQty = roundPurchase(netRequirement, input.packMultiple || 0, input.minimumOrderQty || 0)
  return { confirmedDemand, forecastDemand: roundQty(forecastDemand), residualForecast: roundQty(residualForecast), grossDemand: roundQty(grossDemand), provisionalCoverage: roundQty(provisionalCoverage), physicalNet: roundQty(physicalNet), netRequirement, suggestedQty, suggestQty: suggestedQty, excessQty: roundQty(suggestedQty - netRequirement) }
}

function transferSurplus(input = {}) {
  return roundQty(Math.max(0, nonnegative(input.onHand) - Math.max(nonnegative(input.confirmedDemand), nonnegative(input.forecastDemand), nonnegative(input.reserved)) - nonnegative(input.safetyStock)))
}
