 ...
 *
 * **刻意不限制小数位数**。数量列是 `DECIMAL(14,4)`，系统的最小库存精度就是
 * `0.0001`（见 `unitConversion.round4`），调拨、盘点与单位换算都按这个精度算。
 * 最初把「允许小数」实现成「最多两位」，结果调拨审计用例的数量 `0.0001` 被拦、
 * 整条上线审计专项失败——那是**设计错了，不是用例错了**：把商品级开关做成了精度
 * 收窄，会连带否掉系统已有的能力和存量数据。界面上把录入框的 `step` 提示成两位
 * （`lib/qtyStep.ts`）已经足够表达「一般填两位」，不需要后端硬拦。
 *
