-- ⛔ 已废弃：本表与 warehouse_tasks.shortage_reported_at 已由迁移 134 删除（2026-07-26）。
-- 下面的建表语句保留仅为迁移历史完整性，实际结构已不存在，不要再引用。
-- 废弃原因见 134；将来若重做缺货上报，按那时的需求重新设计表结构即可。
--
-- ⚠️ 原现状注记（2026-07 审计）：本表与 warehouse_tasks.shortage_reported_at 建好后
