...
const DOCUMENT_STATUS_RULES = Object.freeze({
  ...
  inboundTask: {
    ...
    actions: {
      ...
      // 收满才能上架：任务必须已进入「待上架(3)」才允许上架。进入 3 有两条路——全部明细行收满
      // 自动推进，或供应商短装时由 ERP 走「短装结案」(close-receiving) 把剩余未收量作罢。
      // 此前 from 含 2，等于把这条规则交给前端页面隐藏按钮去守：PDA 上架页自 v0.1.2 起就按
      // status<3 拦着，但服务端一直放行未收满的单，接口层可以绕过（本地测试数据里就有
      // status=2 却已上架容器的单）。上架是"货已进库位"的事实，不该在收货尚未了结时发生。
      putaway: {
        from: [3],
        message: '当前状态的任务不能执行上架',
        blocked: {
          1: '任务尚未开始收货，无法上架',
          2: '该收货订单还有商品未收货：请继续收货，或由 ERP「短装结案」后再上架',
          4: '任务已完成或已取消',
          5: '任务已完成或已取消',
        },
      },
