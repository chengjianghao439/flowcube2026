# CLAUDE.md — 历史项目说明与 Claude 兼容入口

> **2026-09-04 起，现行项目工作约定统一维护在 [AGENTS.md](AGENTS.md)。Claude Code 与 Codex 在本仓库工作都应先读取该文件。**
> 功能、修复、配置、接口和流程有改动时，必须第一时间同步更新 `AGENTS.md` 及受影响文档，与实现同一次任务完成；不要另在本文件维护一套现行规则。
> 以下正文保留为迁移前的历史资料。旧计数、工具名、验证结果、“未提交/已部署”等标签只代表当时记录，使用前必须核对当前代码和环境；与现行约定冲突时以 `AGENTS.md` 为准，业务事实以代码为准。
