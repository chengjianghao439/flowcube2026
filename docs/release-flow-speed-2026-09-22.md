...
状态：v0.10.3 三端已发布，线上及安装包实际下载摘要 12/12 通过，GitHub 原包补发布成功。15 分钟目标本轮未达成，完整结果与失败记录见 `docs/release-v0.10.3-result.md`。以下保留各轮排障证据。
...
第四轮 `35676265513` 浏览器部署成功，归档中转 114 秒；生产页面验收 47.6 秒、对账回跳 19.4 秒。随后发现 PDA 的原 SCP 同样受限，约 10 MB 包上传数分钟仅完成约 1 MB，停止尚未发布的上传。PDA/桌面补用相同的 HTTPS/受信中转接收器：指定唯一 APK/EXE 条目、runner 大小和 SHA256，SCP 兜底也复验摘要；桌面 artifact 提前到正式发布前，使中转可取得同一份 Windows CI 产物。浏览器业务服务继续正常运行，后续发布仍等待同 SHA 全套门禁。

发布提交 `846f146` 的浏览器和 PDA 已成功，tag `v0.10.3` 的 EXE 也已发布至官网；线上版本、健康、实际 EXE/APK 下载摘要 12/12 通过。GitHub 上传返回附件 `580338753` 成功，但脚本用 tag 查询未公开草稿得到 404，按 Release ID `393409292` 查询可确认附件摘要与官网相同。修复为草稿按 ID 核对并复用；新增只补发布原包的 workflow，验证来源 tag/SHA、同提交门禁、原 CI artifact/官网/GitHub 三方摘要，禁止重建或重传。原 tag 运行的失败记录不删除、不伪装为成功。相关补发布与线上校验 30 项测试通过。
