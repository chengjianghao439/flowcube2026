# CLAUDE.md（已归档）— 历史项目说明

> **2026-09-18：本文件由仓库根目录 `CLAUDE.md` 归档为 `docs/claude-md-archive-2026-09-04.md`；现行项目工作约定统一维护在 [AGENTS.md](../AGENTS.md)，Claude Code 与 Codex 在本仓库工作都应先读取该文件。**
> **不要在工作区目录链上重新创建 `CLAUDE.md`（根目录尤其不行，一行指针也不行）。** 编码代理的「工作区指令注入」按 `<项目根>/AGENTS.md` → `CLAUDE.md` 的顺序加载候选，并把同目录里**靠后的候选当作"更具体"而优先保留**：根目录那份 166 KB 的 `CLAUDE.md` 会占满字节预算被留下，211 KB 的 `AGENTS.md` 反而被整份丢弃（2026-09-18 实测：新会话注入的正是这份过时正文）。归档后根目录只剩 `AGENTS.md`，注入的才是现行约定。
> 以下正文保留为迁移前的历史资料，章节编号未改动，历史记录里「见 CLAUDE.md 第 X 节」的指引仍指向本文件同一节。旧计数、工具名、验证结果、“未提交/已部署”等标签只代表当时记录，使用前必须核对当前代码和环境；与现行约定冲突时以 `AGENTS.md` 为准，业务事实以代码为准。
