...
// 第二轮审计探针：断言当前缺陷存在，不能作为修复后的通过标准。
test('round2: delayed old 401 replays a write using the replacement session', async () => {
  let release!: () => void
  const pause = new Promise<void>(resolve => { release = resolve })
  const originalAdapter = axios.defaults.adapter as (cfg: InternalAxiosRequestConfig) => Promise<unknown>
  axios.defaults.adapter = async cfg => {
    if (cfg.url === '/sale/901' && cfg.headers.Authorization === 'Bearer old') {
      requests.push({ ...cfg, headers: new AxiosHeaders(cfg.headers) })
      await pause
      throw unauthorized(cfg)
    }
    return originalAdapter(cfg) as never
  }
  const { default: api } = await import('./client')
  const pending = api.delete('/sale/901')
  await vi.waitFor(() => expect(requests).toHaveLength(1))
  session.token = null
  session.refreshToken = null
  session.token = 'replacement-access'
  session.refreshToken = 'replacement-refresh'
  release()
  await expect(pending).resolves.toMatchObject({ data: 'ok' })
  const refresh = requests.find(r => r.url === '/auth/refresh')!
  expect(JSON.parse(refresh.data).refreshToken).toBe('replacement-refresh')
  expect(requests.filter(r => r.url === '/sale/901').map(r => r.headers.Authorization)).toEqual(['Bearer old', 'Bearer new'])
  console.log('ROUND2_SESSION: old write -> replacement session refresh -> replay confirmed; no real HTTP used')
})

test('round2: successful company create is rejected after switching company', async () => {
  const { default: api } = await import('./client')
  let release!: () => void
  const pause = new Promise<void>(resolve => { release = resolve })
  let started = false
  api.defaults.adapter = async config => {
    started = true
    await pause
    return { data: { success: true, data: { id: 3, code: 'PROBE' } }, status: 201, statusText: 'Created', headers: {}, config }
  }
  const pending = api.post('/accounting/companies', { code: 'PROBE', name: 'test' }).catch(e => e)
  await vi.waitFor(() => expect(started).toBe(true))
  company.companyId = 2
  release()
  expect(axios.isCancel(await pending)).toBe(true)
  console.log('ROUND2_COMPANY: successful write response discarded after company switch; no database used')
})
