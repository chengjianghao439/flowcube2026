...
- **数量精度（迁移 254/255）**：**系统的最小库存精度是两位小数（0.01）**——所有数量列统一 `DECIMAL(_,2)`，换算是先折算再按 `unitConversion.roundQty`（= round2）取整；金额、单价、授信额度仍是四位，那是另一回事（单价 8.3333 元/个 是有意的）。`product_items.allow_decimal_qty` 是**商品级**开关：`1`（默认）表示数量可带小数，`0` 表示该商品只能按整数出入库——它不是展示开关，下单、请购、转采购、调拨、退货、盘点、库存调整、拆分、收货与出库都在服务端校验，填小数一律 400（`QTY_INTEGER_REQUIRED`）。校验落点：录入类模块统一走 `foldEntryItems()`（先折算成基本单位再一次性批量校验，新增录入路径必须走它，不能退回手写的 `foldEntryItem()` 逐条循环）；出库、占库、扫码盘点、拆分、收货这些执行类入口也校验整数性。默认值 `1` 是刻意的：存量商品行为零变化。守卫：`npm run test:qty-precision`、`npm run test:qty-precision-coverage`。**2026-09-20 从四位收到两位**（用户要求），存量数据按四舍五入处理；改动涉及的取整点必须先 grep 全仓 `10000`（乘除都要改，只改乘法会让采购建议算出 1.08 而不是 108）。
