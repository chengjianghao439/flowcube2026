...
- **商品数量精度（迁移 254）**：`product_items.allow_decimal_qty` 是**商品级**开关——`1`（默认）表示数量可带小数，`0` 表示该商品只能按整数出入库。它不是展示开关：下单、请购、转采购、调拨、退货、盘点、库存调整、拆分、收货与出库都在服务端校验，「只能整数」的商品在这些入口填小数一律 400（`QTY_INTEGER_REQUIRED`）。**开关只约束整数性，不限制小数位数**：数量列是 `DECIMAL(14,4)`，系统的最小库存精度就是 `0.0001`（`unitConversion.round4`），调拨、盘点与单位换算都按这个精度算；最初把「允许小数」实现成「最多两位」，直接拦掉了调拨审计用例的 `0.0001` 数量——那是设计错了，不是用例错了，把商品级开关做成精度收窄会连带否掉已有能力。界面上录入框的 `step` 提示成两位（`lib/qtyStep.ts`）已经够用。校验落点：录入类模块统一走 `utils/unitConversion.js` 的 `foldEntryItems()`（先折算成基本单位再一次性批量校验，新增录入路径必须走它，不能退回手写的 `foldEntryItem()` 逐条循环）；出库、占库、扫码盘点、拆分、收货这些执行类入口也校验整数性。**不改精度、不回改存量**：历史里的多位小数照旧读得出、算得对。默认值 `1` 是刻意的——存量商品行为零变化；反过来默认 `0` 会让所有按重量/长度卖的商品突然无法下单。守卫：`npm run test:qty-precision`、`npm run test:qty-precision-coverage`。
