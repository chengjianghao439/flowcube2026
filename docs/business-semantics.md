...
- **商品数量精度（迁移 254）**：`product_items.allow_decimal_qty` 是**商品级**开关——`1`（默认）表示数量可带小数且录入最多两位，`0` 表示该商品只能按整数出入库。它不是展示开关：下单、请购、转采购、调拨、退货、盘点、库存调整、拆分、收货与出库都在服务端校验，「只能整数」的商品在这些入口填小数一律 400（`QTY_INTEGER_REQUIRED`）；允许小数的商品在**录入类**入口超两位是 `QTY_DECIMAL_SCALE_EXCEEDED`。校验落点分两类：录入类模块统一走 `utils/unitConversion.js` 的 `foldEntryItems()`（先折算成基本单位、再一次性批量校验，所以新增录入路径必须走它，不能退回手写的 `foldEntryItem()` 逐条循环）；执行类入口（出库/占库/扫码盘点/拆分/收货）**只校验整数约束、不校验两位上限**——那些数量常常直接来自存量或既有单据，历史数据里可能带三位以上小数，按两位拦会把正常业务卡死。数量列本身仍是 `DECIMAL(14,4)`，**不改精度、不回改存量**：存量里已有的多位小数照旧读得出、算得对，只是不能再这么录。默认值 `1` 是刻意的——存量商品行为零变化；反过来默认 `0` 会让所有按重量/长度卖的商品突然无法下单。守卫：`npm run test:qty-precision`（校验函数本身）、`npm run test:qty-precision-coverage`（会写数量的入口是否都接上了）。
