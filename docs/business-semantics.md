...
- **可承诺与当前可拣分开**：销售占库预览的 ATP 可承诺量仍按上式计算；“当前未锁容器现货”只汇总同仓未被其他任务整容器独占的 ACTIVE 容器，作为作业可执行性参考，不增加或减少合法预占。拣货推荐无候选时按同仓范围返回至多 3 个占用任务摘要，PDA 提示等待任务完成或逆向归还后刷新，不指引盲扫已锁容器。扫描与出库仍由事务内的容器锁和预占检查最终裁决。独立库回归见 `smoke:concurrency-guards` 的 5 件容器 A 拣 3、B 占 2 场景。
