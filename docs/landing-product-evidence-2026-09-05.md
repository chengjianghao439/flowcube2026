...
3. 供货示例：`BusinessStory.tsx`（2026-09-12 替代原 SupplyStory） 以 80 件订单解释“现货 40 / 待兑现采购 40 → 收货待上架仍为 40 / 40 → 上架后实物支撑 80 / 预计 0”。三阶段是固定文案与数量图示，不复制后端 ATP 算法；假设无其他库存变动。
4. 六类能力：采购补货、销售信用、仓储追溯、盘点库存健康、资金会计、岗位审批，并提供现有系统入口。
5. 2026-09-12 新版将六类业务能力以可展开说明展示；不再单独展示旧版三类异常问答。退货部分质检等事实仍由五场景示例解释。
6. 三端分工、版本记录及下载保留。
