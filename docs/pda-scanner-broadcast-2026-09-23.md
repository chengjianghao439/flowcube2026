...
- v0.10.6 的原生广播桥默认关闭，供 i6310pro 真机验证。现场确认后，下一版在插件首次加载时默认启用，以 `RECEIVER_NOT_EXPORTED` 注册厂商广播接收器并切换输出模式；非厂商设备失败时保留键盘路径。「库存查询」仍可在本次运行中主动关闭/重新开启。退后台时暂时恢复原键盘模式，回前台时重新启用；销毁或异常退出后恢复设备原模式。切换前同步记录待恢复状态，进程异常退出后，下次启动先恢复再启用。
...
- 不给作业页增加隐藏输入框或自动焦点；`PdaScanner` 手输框已打开时，硬件广播扫码仍可完成当前操作。按用户真机反馈，收货上架页允许点扫码区域后手动输入完整库存/库位条码，仍经过原有条码解析、单据归属、状态及库位校验；调拨出入和销售退货上架暂保持强制扫码。

## 验证边界

- 前端原生事件、键盘/广播去重及手输框状态有组件测试；Android 模式恢复和广播内容校验有 JVM 测试。Debug APK 本地编译验证只证明可构建。
- i6310pro 上 v0.10.6 已由用户确认：主动开启后，无焦点硬件扫码在 PDA 端正常；上架页的提示框原本不是输入框，点它不会弹键盘。默认启用与点上架扫码区手输是下一版改动，仍须在新 APK 真机安装后分别验收。跨应用伪造广播拒收仍需具备外部测试应用时验证；`RECEIVER_NOT_EXPORTED` 不改为无保护的 `RECEIVER_EXPORTED`。
- 远程真机验收：更新后不进库存查询开关，直接到上架任务扫库存条码与库位条码；确认页面初始不弹键盘，点扫码区域才弹键盘，手输后仍走原有校验。离开极序 Flow 后确认记事本仍可扫码输入。
