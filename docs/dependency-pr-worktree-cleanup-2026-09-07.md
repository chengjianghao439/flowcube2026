# 历史依赖 PR 与旧工作树清理（2026-09-07）

初次核对基线：`main` / `2cd4589a8da8121bc8cfe2455e5b584afbe80e50`。按用户要求处理 22 个 Dependabot PR，并核对、清理 Claude 旧工作树；后续用户授权清理上线审计工作树时，main 为仅增加本清理说明的 `2952276`。此次不升级依赖、不修改业务逻辑，不合并 PR，不发布版本。
...
用户在了解 97 项改动的归属及与当前主线的差异后，明确要求按归档清理建议处理。

- 原目录：`/Users/chengjianghao/.config/superpowers/worktrees/flowcube/prelaunch-fixes-20260905`；原分支：`codex/prelaunch-fixes-20260905`；原 HEAD：`16d6435f7ca7d64db9641ede82df4c2b76c5c7b1`，为 main 祖先，无独有提交。
- 原状态为 67 个文件修改、2 个文件删除、28 个未跟踪文件，共 97 项。此前交付曾将这 97 个路径同步回主目录，修复已随 `b5f066b` 纳入主线；旧工作树没有同步收尾。
- 与当前主线比较：65 项一致，14 项与主线历史文件内容一致，18 项不同；后者含两项空白差异，以及后续增加的会话隔离、工资跨月依赖保护、连接池等待超时、PDA 浏览器凭据初始化与 CI 门禁等。保留当前主线，不用旧副本覆盖后续修复。
- 删除前核对无进程打开该目录内文件，将全部源码、未跟踪文件与 `frontend/dist` 归档，排除可重新安装的 `node_modules` 和工作树 `.git` 指针；逐文件 SHA-256/符号链接校验通过，删除前再次检查文件内容和 Git 状态未变化。
- 归档目录：`/Users/chengjianghao/.config/flowcube/cleanup-backups/20260907-prelaunch-audit`（权限 700）；归档文件 `prelaunch-worktree.tar.gz`（权限 600），1,762 个文件、3,824,647 字节。
- 归档 SHA-256：`3f24d13f3cb4f7f8fa54bb5ab36c4bd12b3e56f08a856340328b62e556583ea4`。同目录保留原 HEAD、Git 状态、二进制 patch、忽略路径、逐文件清单及归档校验结果。
- 已删除旧工作树和对应本地分支。恢复时在原 HEAD 新建独立工作树，先应用 `tracked.patch`（包括两项文件删除），再解入归档恢复未跟踪文件及产物；仅解压不能恢复文件删除状态。不要覆盖当前 main。其余发布工作树保留。
...
- `prelaunch-fixes-20260905` 最初保留，随后按用户追加授权完成上述归档与清理；其他发布工作树保持原样。此次续作只校验文件、归档、Git 状态和文档差异，没有重复执行之前的依赖审计或业务回归。
