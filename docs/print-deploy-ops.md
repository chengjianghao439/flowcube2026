...
- **不新增付费云资源的可选本地中转**：`FLOWCUBE_RELEASE_RELAY=1 FLOWCUBE_RELAY_SSH_TARGET=<SSH别名> FLOWCUBE_RELAY_PROXY=<已有本地代理地址> npm run release:prod`。Mac 需保持在线；脚本自动启动受限时长的 watcher，绑定同 SHA、push 事件、main/tag、工作流和 run attempt，持续补位下载并刷新 GitHub 短期下载地址，校验 ZIP 摘要和唯一成员，再经 SSH 复用连接交付原包。生产接收器仍独立核对 runner 提供的大小/SHA256。退出清理本任务进程、临时文件和 SSH socket；只有接收工作流结束才清理已交付文件。原直连与 SCP 回退仍保留，必须记录实走路径，不能把回退成功称作本地中转全通过。无需新增 npm 依赖。OSS 方案已按用户免费优先要求暂缓，未开通、未创建云权限。 v0.10.4 已完成三类产物自动中转与线上 12/12 核验；包含首轮修正及耗时证据见 `docs/release-v0.10.4-result.md`。
