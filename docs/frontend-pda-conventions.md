...
- 所有前端（桌面、浏览器、PDA）的用户列表、导出、用户选择器都以 `hideDevelopment=1` 隐藏明确用于开发验收的账号：`codex_`、`smoke_`、`esc_`、`pc_` 前缀及 `cua_pda_test`。服务端在分页和总数统计前过滤；不带参数的后端接口保留完整数据供测试使用，不停用或删除账号。部门负责人若引用开发账号，列表不返回其姓名，表单只提示负责人不可用并要求更换或清空；历史关联不被静默改写。开发账号登录时，顶栏、个人信息和 PDA 问候语只显示中性身份。命名依据见 `scripts/repair-dev-test-data.cjs` 与 `docs/local-preview-acceptance.md`。
...
- 用户管理新增/编辑弹窗按“账号信息、访问权限”分区，窄窗口单列；新增时必须主动选角色，不能预选首个可分配角色。字段错误就地提示，提交失败留在弹窗；编辑超管仍不传角色，非超管仍不传自行审批设置。列表仅向超管提供修改内置超管账号的入口；搜索后的导出沿用当前关键字。网页删除确认框在请求失败时保留以便重试；桌面原生确认框点击后即关闭，失败后释放目标，允许从列表重新发起。
