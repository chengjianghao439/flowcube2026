...
- **i6310pro 硬件扫码接收**：v0.10.6 已由用户真机确认主动开启后无焦点硬件扫码覆盖 PDA 作业页。下一版原生 `PdaScanBridge` 首次加载时默认启用受保护的厂商广播，`usePdaScanner` 送往现有扫码页面；键盘事件路径继续用于浏览器预览及非厂商设备。「库存查询」允许主动关闭和重新开启；退后台恢复原键盘模式。不能为修复无焦点扫码而给作业页加隐藏输入框；真机验证边界见 `docs/pda-scanner-broadcast-2026-09-23.md`。
- **收货上架的手动条码入口**：扫码区域默认不聚焦，直接按硬件扫码键；只有用户点该区域才打开并聚焦手输框。手输库存条码、库位条码与硬件扫码经过同一流程及服务端校验。其它 `PdaScanner` 页面保持原有「手动输入」按钮或强制扫码方式，不因这项上架需求整体改动。
