...
- 前端（桌面、浏览器、PDA）的用户列表、导出、用户选择器默认以 `hideDevelopment=1` 隐藏明确用于开发验收的账号：`codex_`、`smoke_`、`esc_`、`pc_` 前缀及 `cua_pda_test`。用户管理页仅向超级管理员提供“显示开发账号”开关，开启后列表与导出均包含这些账号；服务端在分页和总数统计前过滤。不带参数的后端接口保留完整数据供测试使用，不停用或删除账号。部门负责人若引用开发账号，列表不返回其姓名，表单只提示负责人不可用并要求更换或清空；历史关联不被静默改写。开发账号登录时，顶栏、个人信息和 PDA 问候语只显示中性身份。命名依据见 `scripts/repair-dev-test-data.cjs` 与 `docs/local-preview-acceptance.md`。
- 所有前端的角色列表、新增/编辑用户角色选择及审批流角色选择按角色编码隐藏 `smoke_` / `codex_` 开发角色；不能用 `is_system` 判断，因为开发角色可能也被标为系统角色。历史审批流引用隐藏角色或用户时只显示中性占位并保留原 ID，保存时不悄然改派。新岗位预置角色见迁移 `257_seed_job_role_presets.sql`，角色权限页可调整授予项。
- 用户管理新增/编辑弹窗按“账号信息、访问权限”分区，窄窗口单列；新增时必须主动选角色，不能预选首个可分配角色。字段错误就地提示，提交失败留在弹窗；仅超级管理员可在编辑时修改登录账号，原密码不可查看，需要更换时使用“重置密码”。编辑超管仍不传角色，非超管仍不传自行审批设置。列表仅向超管提供修改内置超管账号的入口；搜索后的导出沿用当前关键字及开发账号开关。网页删除确认框在请求失败时保留以便重试；桌面原生确认框点击后即关闭，失败后释放目标，允许从列表重新发起。
- 部门管理的树表行把业务字段放在 `dept` 内；负责人姓名、创建时间等列须从该对象读取，不能从仅有 `id/dept/depth` 的外层行取值。新增、修改、删除部门后同时刷新完整部门树与 `department-options` 缓存，使已挂载的用户新增/编辑弹窗及时看到最新部门名称和选项；部门选项接口仍允许没有 `department.view` 的用户管理场景读取。
- 部门列表标明普通账号负责人的失效状态、审批流引用数与“直属成员”口径；直属成员数不计开发账号，有权限者可从引用数进入审批流配置。树表支持行内新增子部门、全展开/收起，搜索结果展示完整部门路径；展开按钮暴露展开状态并支持方向键展开/收起。新增/编辑弹窗窄窗口单列，已禁用负责人只展示历史选中值而不可新选，字段错误就地提示，保存期间防重复提交。用户变更须刷新 `user-options` 与 `departments`；审批流配置变更须刷新 `departments` 引用数。
- 仓库访问范围弹窗必须同时取得当前授权与仓库列表后才能勾选、保存；读取失败提供重试，不能把尚未加载的空选择当成“不限仓”提交。切换目标用户先清空上个用户的选中状态，再用当前用户授权回填。
- 顶栏“我的信息”与“我的仓库权限”走仅返回登录者本人的自助接口，普通员工不需要用户管理查看权限即可查看自己的资料。
- 客户、供应商表单只提交名称、联系与结算等可编辑字段，新增与编辑均不提交 `code`；编号由服务端创建并保持，编辑态仅显示已有编号。新增成功后刷新对应 `customers` / `suppliers` 列表缓存，列表重新查询可见新编号。
- 供应商新增/编辑错误由 `BaseCrudPage` 提示一次：对应 API 调用使用 `skipGlobalError`，组件读取 `ApiClientError.message` 展示后端中文原因，前端手机号校验的响应文案也保留。客户新增/编辑仍由 `useCustomers` 的错误回调单独提示；重复名称或非法手机号时不要叠加全局 toast。
- 开发构建的页面错误只显示通用恢复提示，堆栈继续走错误上报；强制更新弹窗不暴露调试标记。承运商页面不提供 mock 平台选项，历史 mock 配置只显示中性“不可用平台”。官网版本摘要使用业务用户能理解的功能文案，不展示扫码试验及内部状态值。
- 操作日志页面及其导出隐藏开发账号作为操作人的记录；这只改变前端展示，后端缺省接口仍保留完整技术日志。
- 操作日志页面及其导出默认隐藏打印客户端成功心跳和领取轮询。打印任务以打印记录页的任务状态为准；操作日志保留打印请求错误与完成、失败回执，历史技术日志仍可由后端缺省查询排查。
- 成功登录与有效退出的操作日志以服务端已核验身份为准，列表显示“登录成功”“退出登录”；失败登录、伪造或重复退出保持匿名，请求体中的用户名不可当作可信操作人。详情仍保留方法与路径，敏感凭据不得落日志。
- 商品查找器在占库、释放或取消、出库、上架、撤回收货、盘点、调拨、退货、处置及库存调整后失效库存缓存；切换仓库时不展示上一仓的占位库存。可用量以服务端重新查询为准，提交仍由后端事务裁决。
- 盘点详情的保存、提交、刷新账面和取消失败必须显示可理解的业务原因，保留弹窗与未保存实盘输入；页面负责提示时禁用全局重复错误提示。
- ERP 浏览器直达 `#/pda/*` 与独立 PDA 构建都先读取本机设备缓存，再渲染登录、绑定及作业页。缓存读取损坏时提示重新绑定，不把本地故障误报为服务端密钥无效；浏览器调试版缓存仅内存保存，刷新后重绑，Android 原生加密存储仍需真机验证。
...
- 销售作业进度卡对执行中、未绑格任务显示“待分配分拣格”；分拣格管理页只向有仓库任务分配权限的主管提供补分配弹窗，按当前仓库筛选待处理任务，提交稳定请求键并刷新格位、任务与销售详情。PDA 扫商品发现任务无格时提示联系主管，补分配后刷新并重新扫商品码，再扫格码确认；不由 PDA 自动占格，也不跳过扫码闭环。
...
- 月结供应商“全部账款”待确认应付显示“待确认”状态和“确认结算”主操作；只有具备 `payment.confirm` 权限才出现该操作，原单/收货单仍可从次操作进入。已确认应付与客户月结不显示确认按钮。新建对账单候选用原账款 `id` 选择、全选和提交，已建详情的 `recordId` 是独立字段；到期日用北京时间业务日期展示，不能显示原始 ISO 时间戳。
- 客户和供应商批量导入页的帮助文案与 Excel 模板一致：结算方式可填 `现结`/`1` 或 `月结`/`2`，留空默认为月结；无效非空值显示逐行错误，不再提示已淘汰的预付定金、货到付款选项。导入成功后列表查询失效并重新读取服务端数据。
...
- **i6310pro 硬件扫码接收**：v0.10.7 原生 `PdaScanBridge` 首次加载时默认启用受保护的厂商广播，`usePdaScanner` 送往现有扫码页面；键盘事件路径继续用于浏览器预览及非厂商设备。用户已在真机确认默认直接扫码、上架页点扫码区手输及退后台后记事本键盘扫码正常。库存查询的临时扫码模式开关已随 v0.10.8 移除，统一扫码条与「手动输入」按钮也已随该版发布；本版 i6310pro 真机操作仍待单独验收。退后台恢复原键盘模式。不能为修复无焦点扫码而给作业页加隐藏输入框；真机验证边界见 `docs/pda-scanner-broadcast-2026-09-23.md`。
...
- **PDA 页面高度**：`PdaLayout` 外层已有 `100dvh` 最小高度及上下安全边距；内层页面的 `min-h-screen` 在 PDA 范围内扣除这两处边距，避免内容未满屏时仍能向下滑动一小段。内容超过一屏时继续正常滚动，不能靠隐藏溢出裁掉操作区。
- **PDA 实体返回键与息屏**：Android `MainActivity` 不拦截返回键、不强制屏幕常亮；独立 PDA 路由的 `PdaHardwareBack` 接收 Capacitor 返回事件，作业页返回上一页（没有历史时回工作台），工作台和登录页退到后台。原先为浏览器共用路由插入的历史占位仅用于非原生环境，不能在 Android 上吞掉实体返回。屏幕超时遵从设备系统设置，不由应用无限延长。
- **PDA 操作反馈只显示一条**：扫码与作业结果由页面的 `PdaFlash` 显示，已由页面接管的 API 请求设置 `skipGlobalError`，避免同一错误再从右侧全局 toast 弹出；条码刚被读取时不提前显示绿色成功，等业务校验完成后再反馈。上架页的流程反馈与回执确认反馈共用一处 `PdaFlash`；其它未由页面接管的 PDA 全局提示居中悬浮且最多显示一条。保留声音与震动，但必须有可见的结果提示。扫码回归保留真实键盘监听和流程引擎，见 `frontend/src/pages/pda/putaway.test.tsx` 与 `docs/pda-putaway-scan-investigation-2026-09-22.md`。
...
- **PDA 拣货商品汇总**：单列卡片在商品编码、名称之外显示待拣数量与单位、已拣/共需、关联单数；有值时补充型号、颜色、供应商型号。关联单数按不同业务单计（销售单按 ID、退货单按类型及 ID），同单多任务仍展开任务选择；无单据归属的任务按任务计。只有一个任务时沿用原任务启动流程直接进入；多个任务先显示销售单号、客户、仓库和任务状态，再由用户选定任务。汇总接口一次返回所有关联任务的选择信息，不受任务池首页 50 条上限或 `GROUP_CONCAT` 长度限制。订单列表与商品汇总的数量最多显示两位有效小数，不取整。
...
### PDA 作业交互复核（2026-09-23，本地待发版）

- PDA 工作台入口的权限集合必须与目标路由 `required` 一致；上架需要收货查看和上架执行，拣货、复核、打包均需要任务查看和对应执行权限。岗位徽标优先显示用户实际角色名称，不把未知角色 ID 猜成拣货员；开发账号身份沿用 `visibleAccountIdentity` 隐藏。
- 任务列表请求失败要显示「加载失败」及当前页重试，不能用空数组显示「暂无任务」。收货、调拨、销售退货、盘点、拣货商品视图、复核、打包、改单和拣货退回均按此处理；上架、改单及拣货退回详情读取失败也提供重试。
- PDA 登录密码可见性按钮须有动态可读名称并可用键盘聚焦；页面允许用户缩放。网络待确认黄条用深色文字。返回、刷新、扫码手输/确认/取消、盘点移除和工作台检查更新等小按钮保持至少 44px 高的触控区域。
- 上架偏离推荐库位的第二次同码实扫，只有在等待该确认时才允许同源扫码在 300ms 后再次提交；双通道回放和更短的硬件抖动继续拦截。手动输入仍只由显式按钮触发。
...
## 2026-09-24 操作记录与单据活动

- 操作记录页按服务端页码查询，每页 20 条；API 显式使用 `listMode: 'summary'`，筛选变化时回到第一页，不自动抓取至 5000 条。
- 写请求在进入业务路由前先保存状态码为空的操作意图，响应发送前补全状态与单据事件。进程中断或补全失败留下“待确认”记录供排查；预写失败则不执行业务路由。打印客户端心跳与领取轮询仍按原有过滤规则处理。
- 单据已有业务创建事件时隐藏同单据的通用“创建单据”事件；关联仓库任务按类型及最多 100 个 ID 一批读活动记录，避免逐任务查询。
