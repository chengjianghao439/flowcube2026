...
- **PDA 聚焦规则按「这个页面要不要输入」判断**（2026-09-19 用户明确该依据）：**登录页需要输入，自动聚焦账号框是对的**；**扫码作业页不需要输入，进页面与扫码结束都不得自动聚焦输入框**（软键盘会挡住扫码视野），只有用户点「手动输入」按钮才渲染手输框并聚焦。统一用 components/pda/PdaScanner.tsx，该组件不再提供 autoFocus；判断新页面时套用同一条依据（需要键入的可聚焦，扫码/浏览类不得聚焦），不要按「统一体验」把两类页面拉平。`tests/pda-scan-focus-contract.test.js`（`npm run test:pda-scan-focus`）机械守住：除 `login.tsx`（需要输入，`autoFocus` 是正面实现）外的 PDA 页面不得出现 `autoFocus`，且 `PdaScanner` 里每处 `.focus()` 必须自身带 manual 语义——聚焦只能由点「手动输入」触发（反向验证：给作业页加 `autoFocus`、或在挂载 effect 里自动聚焦，都必须失败）。见 docs/pda-scan-default-mode-2026-09-17.md。
- **i6310pro 硬件扫码接收**：v0.10.7 原生 `PdaScanBridge` 首次加载时默认启用受保护的厂商广播，`usePdaScanner` 送往现有扫码页面；键盘事件路径继续用于浏览器预览及非厂商设备。用户已在真机确认默认直接扫码、上架页点扫码区手输及退后台后记事本键盘扫码正常。库存查询的临时扫码模式开关已在本地页面代码中移除，已发布的 v0.10.7 仍显示该开关，须待后续 PDA 版本发布才会消失。退后台恢复原键盘模式。不能为修复无焦点扫码而给作业页加隐藏输入框；真机验证边界见 `docs/pda-scanner-broadcast-2026-09-23.md`。
- **PDA 扫码条统一交互**：拣货、收货上架、分拣、拣货退回、改单确认、盘点、库存查询及其它允许手输的 `PdaScanner` 页面统一使用底部扫码条。默认不渲染或聚焦条码输入框，直接按硬件扫码键；只有点扫码条内的「手动输入」按钮才打开并聚焦手输框，点扫码提示文字不弹键盘。手输条码与硬件扫码走相同的业务处理与服务端校验；盘点数量、收货批次等业务字段仍按原流程手填。原本强制扫码的页面仍不提供手输入口。拣货页输入区上方重复的箭头提示卡已移除。
- **收货扫码上架反馈必须可见**：`putaway.tsx` 同时渲染 `usePdaFlow` 的 `engine.flash` 和回执确认操作的 `flash`，显示库存条码校验、接口错误、推荐库位和成功提示；不能只调用声音/震动而遗漏视觉反馈。扫码回归保留真实键盘监听和流程引擎，见 `frontend/src/pages/pda/putaway.test.tsx` 与 `docs/pda-putaway-scan-investigation-2026-09-22.md`。
...
- **ERP/PDA 文本显示**：ERP 业务字段、备注、异常原因、分类路径和选择结果允许换行，禁止省略号和行数截断；PDA 区分总览与详情，规则见下一条。公共表格按内容增高，选择框按内容增高，工作区标签展示完整名称并横向滚动。图表采用多行坐标标签，增加图高并允许滚动。`frontend/src/lib/textWrapping.test.ts` 以语法树检查样式，仅精确允许 `PdaOverviewText` 的总览辅助文字截断，并包含反向验证。
- **PDA 编码优先、总览与详情分层**：拣货汇总保持单列。商品总览使用 `PdaProductIdentity view="overview"`：编码在首行加粗完整显示，商品名称作为次要信息单行省略；任务列表突出单号，客户/供应商/仓库等辅助文字可用 `PdaOverviewText`。进入商品作业或任务详情使用 `view="detail"`，名称、规格等完整换行；编码与条码不省略。盘点商品全名放在详情正文，顶部优先编码。详见 `docs/pda-code-first-2026-09-22.md`。
...
> **后端的 `AppError` 消息同样算用户可见文案**：它们经 `toast.error(e.message)` 直接弹给用户，受同一套禁用词约束。`npm run test:copy-conventions` 会扫 `backend/src` 里每条 `new AppError('…')` 的消息文本（注释与标识符里的领域词不管）。2026-09-20 补这条之前，后端积了 66 处「容器」、8 处「波次」、4 处「会话/快照/回执」——前端早就改干净了，用户点一下还是能看到「容器不存在」。
...
- **详情页的标签用业务单号，不用数据库主键**：`routePatterns` 的兜底名（「销售单 #3260」「运单 #12」）里的数字是主键，用户在标签栏上认不出是哪张单，也与「从列表点进来」时不一致（列表入口传的是 `orderNo`）。详情页数据到位后调 `useWorkspaceTabTitle(order?.orderNo)` 换成单号；两个实现约束：用 `updateTabTitle`（不能 `addTab`，否则数据晚到会把用户从别的标签拽回来）、路径取 `TabPathContext`（不能用 `location`，keep-alive 下非激活标签同样挂载会把标题写错标签）。首帧缓存已命中时必须等待父级注册标签后再写单号；静态列表标题按现行路由名更新，不能把历史名称误当单号保留。
...
**选择器统一**：「从弹窗里挑一条记录」的字段一律用 `components/shared/PickerField`——外观是**输入框样式 + 已选时的清除 X**，点击弹出 Finder。它合并自两个组件：表单用 `FinderTrigger`（做成输入框样、无清除），查询弹窗用 `QueryPickerField`（做成按钮样、带清除），于是同一个「选客户」在销售单表头与销售查询弹窗里长得不一样，用户不知道这里能不能打字。高度由 `className` 决定：查询弹窗传 `h-9`（与同排的 Select / Input 对齐），表单用默认 `h-10`。禁用状态同时覆盖主按钮和清除按钮。**真需要输入关键字筛选的场景（如客户列表搜索、超额放行页搜索）仍用原生 `Input`**，不要拿 PickerField 冒充输入框。

**金额显示统一**：金额一律走 `lib/format` 的 `money()`（带 `¥`）或 `amount()`（会计凭证借贷方，不带符号），不得手写 `¥{x.toFixed(2)}`——后者没有千分位，`¥1234567.89` 在列表里读不出位数；空值还会显示成 `¥0.00`，让「没有数据」看起来像「金额为零」。统一后空值一律 `—`，负数一律 `¥-20.00`。仪表盘 `components/dashboard/chartTheme` 的 `money` 就是 `lib/format` 的再导出，不要再另存实现。**数量走 `qty()`，与金额分开**：数量是 `1.2 公斤`，写成 `1.20` 是错的（金额正好相反）。`lib/format.ts` 的 `qty()` 整数就显示整数、小数保留有效位（上限 2 位，对应库存精度 `DECIMAL(*,2)`）、空值 `—`。此前全仓有 8 处各自定义的 `formatQty`/`fmtQty`/`num`/`qty`，至少 4 种口径——固定两位的把 `1.2` 显示成 `1.20`，`String(n)` 的连千分位都没有。

**唯一入口由守卫钉住**：除 `lib/format.ts` 外，任何文件都不得自己配 `minimumFractionDigits`/`maximumFractionDigits`——金额用 `money()`/`amount()`，数量用 `qty()`。上一条只拦 `¥` + `toFixed(2)`，漏掉了同义的 `toLocaleString` 写法（台账、发票、会计报表、退款、处置就是这么留下来的），改成拦特征配置项后换写法也拦得住。

**打印管线走同一口径**：单据表格的单价/金额列、合计行、单头合计，以及模板编辑器预览的明细与样例数据，全部是 `money()`——客户拿到的销售单、供应商拿到的采购单同样是给人看的，没有理由不带千分位。唯一的刻意例外是以「万」为单位的库存价值卡片（`¥1.23万`，位数本来就短），逐条登记在 `tests/copy-conventions.test.js` 的 `MONEY_ALLOW`，失效即守卫失败。单价 tooltip 里的 `toFixed(4)` 是精确值，不套用金额格式。

**数量输入联动商品精度**：商品可以设「只能整数」（迁移 254），因此数量输入框的 `step` 一律走 `lib/qtyStep.ts` 的 `qtyStep(allowDecimal)`，不再写死 `step="0.0001"`。某一行是否允许小数用 `hooks/useProductQtyPolicies.ts` 按需批量查（每批最多 500 个正整数 ID，空列表不查询；5 分钟缓存，商品保存时随 `products` 查询一起失效，与明细行数据来源无关），**查不到时按允许兜底**——宁可让用户先填、由服务端拦下，也不要因为一次查询失败就把整数框锁死。`step` 本身**不是校验**，它挡不住键入；所有数量框必须显式使用 `<Input quantity>`，在输入、粘贴及失焦提交前校验有效小数位，三位及更多直接拒绝并提示，不悄悄四舍五入；尾零不计入有效位，整数商品同步按 `step=1` 拒绝小数。销售订单提交与占库/释放/出库分配还须校验草稿。后端 `utils/qtyPrecision.js` 独立校验。金额、单价、换算率不得加 `quantity`，换算率展示保留原精度。采购单与 PDA 拆分原有只收整数的规则保持不变。
