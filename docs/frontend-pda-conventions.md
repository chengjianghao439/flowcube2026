...
- **i6310pro 硬件扫码接收测试**：Android 原生 `PdaScanBridge` 默认关闭，只在「库存查询」主动开启无焦点扫码测试后接收厂商广播并经 `usePdaScanner` 送往现有扫码页面；键盘事件路径继续用于浏览器预览及非厂商设备。不能为修复无焦点扫码而给作业页加隐藏输入框。关闭测试或退后台恢复原键盘模式；真机验证边界见 `docs/pda-scanner-broadcast-2026-09-23.md`。
