...
- **ERP/PDA 文本完整显示**：业务字段、备注、异常原因、分类路径和选择结果允许换行及连续编码断行，禁止省略号和行数截断。公共表格按内容增高；选择框及 PDA 标题栏采用最小高度；工作区标签展示完整名称并横向滚动。库存价值、仓库分布及账户余额图表采用多行坐标标签，按最长标签增加图高并允许滚动，不裁剪名称。`frontend/src/lib/textWrapping.test.ts` 以语法树检查样式并包含反向验证；详见 `docs/text-wrap-2026-09-22.md`。
- **PDA 拣货商品显示**：拣货任务的商品汇总采用单列；列表和拣货详情的商品名称、编码、货号、规格及颜色自动换行，连续长编码允许断行，不使用省略号或行数截断。
