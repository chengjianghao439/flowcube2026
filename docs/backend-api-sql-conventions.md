...
- **客户、供应商导入的结算方式单独严格解析**：CSV/XLSX 列仅接收 `现结`/`1`→CASH、`月结`/`2`→MONTHLY、空→默认 MONTHLY；其他非空值返回带行号的错误且该行不入库。先校验再取自动编号。历史数据读取继续使用 `normalizeSettlementType` 的旧值兜底，不把它当导入校验器。真实 multipart HTTP 回归为 `npm run smoke:masterdata-import`。
