...
本文档已从 `docs/02-flowcube-流程测试矩阵.md` 移入 `docs/legacy/`。

当前生效文档见：`../02-flowcube-流程测试矩阵.md`。

请勿继续修正本文档中的状态数字。本文档保留历史上下文，其中 `warehouse_tasks.status`、`sale_orders.status`、销售接口路径均已过期。

---

# 已过期

本文档自 2026-04-14 起标记为过期。

过期原因：
- 销售主链路仍写为 `POST /api/sale/orders/{id}/confirm`，但当前真实路由已调整为 `POST /api/sale/:id/reserve`。
- 采购主链路仍按“采购单直接收货”描述，但当前真实实现已转为 `inbound_tasks` 收货/上架闭环。
- 打印、条码、收货状态口径已与本文档部分章节不一致。

使用原则：
- 本文档仅保留历史测试结构参考，不可直接作为当前版本验收依据。
- 当前测试与联调请以真实路由、真实状态机和代码实现为准。
- 建议后续补一份 `流程测试矩阵 v3`，专门覆盖单租户 + inbound_tasks + reserve + warehouse_tasks 主链路。
