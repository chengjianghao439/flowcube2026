...
- **当前迁移号到 180**（06=164-167、04=168-171、11=172-174、03=175、10 Phase0=176-178、10 Phase1=179-180）。新迁移取当前最大 +1（`ls backend/src/database/*.sql | sort | tail -1`），不写死。
- 全量回归基线：mainline 49 / p0 41 / p1 44 / concurrency 83 / finance 103 / integration 96 / warehouse-scope 24 / serial 40 / 权限 166 / test:accounting 7 / smoke:accounting 11 一致（10 Phase0+1 后权限 161→166）。
...
- 新权限码要**三处同步**（后端 permissions.js + 前端 permission-codes.ts + seed 迁移），跑 `test:permissions`（当前前后端各 166 个一致）。
