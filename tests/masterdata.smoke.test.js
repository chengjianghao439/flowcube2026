...
async function main() {
  ...
  try {
    ...
    await pool.query('UPDATE sys_users SET is_active=0 WHERE id=?', [denied])
    await expectStatus(dep, '禁用负责人不能新建', '', 'POST', { name: `${mark}禁`, managerId: denied }, 400)
    await expectStatus(dep, '禁用负责人不能更新', `/${parent.id}`, 'PUT', { managerId: denied }, 400)
    await pool.query('UPDATE sys_users SET is_active=1 WHERE id=?', [denied])
    ...
    const referenced = await create(dep, 'sys_departments', { name: `${mark}引用` })
    const flowId = await insert('approval_flows', { biz_type: 'expense_claim', name: `${mark}审批`, min_amount: 0, is_active: 0 })
    await insert('approval_flow_steps', { flow_id: flowId, step_order: 1, approver_type: 2, department_id: referenced.id })
    const referencedRow = (await request(dep)).body.data.find(r => r.id === referenced.id)
    check(dep, '停用审批流仍计入部门引用', referencedRow.approvalFlowCount, 1)
    await expectStatus(dep, '审批流引用禁止删除', `/${referenced.id}`, 'DELETE', undefined, 409)
    await pool.query('DELETE FROM approval_flow_steps WHERE flow_id=?', [flowId])
    await pool.query('DELETE FROM approval_flows WHERE id=?', [flowId])
    await expectStatus(dep, '解除审批流引用后可删除', `/${referenced.id}`, 'DELETE', undefined, 200)
    ...
    try {
      ...
      const order = ['inventory_logs', 'purchase_returns', 'purchase_orders', 'warehouse_tasks', 'sale_returns', 'sale_orders', 'sale_customers', 'supply_suppliers', 'product_items', 'product_categories', 'approval_flow_steps', 'approval_flows', 'sys_departments', 'inventory_warehouses', 'sys_users', 'sys_roles']
